...
| 2026-08-27 | 정합. 금액 컬럼 numeric | `0079_amount_columns_numeric.sql` — 금액 3컬럼 **bigint → numeric** 전환(수량 계열 numeric[0012·0013·0015]과 타입 대칭). 대상: `transactions.total_amount`·`drugs.current_amount`·`inventory_stock.current_amount`. `ALTER COLUMN ... TYPE numeric USING ...::numeric`(정수→numeric 무손실·default 0 유지·NOT NULL/제약/인덱스 없음). 배경: 금액 bigint라 소수 금액이 **프론트 직접 insert에서 22P02**(문자열→bigint) 실패·RPC는 암묵 반올림 → 계산 시점 Math.round/round(PR #242·0078) 우회 중이나 프론트(half→+∞)·DB(half away) **음수 half 1원 편차** 잔존. numeric 전환으로 반올림 불필요·편차 소멸. **영향조사(Step 0)**: current_amount 기입 함수/트리거 **0개**(0009 apply_tx/revert_tx 미참조·drugs.current_amount 합=0 레거시), total_amount는 bulk_stock_adjust만 참조, 뷰 0·인덱스/제약 0. ★ **monthly_snapshots amount 5종은 이미 numeric이라 범위 밖**. 프론트: numeric은 이 스택에서 **JS number로 역직렬화**(이미 numeric인 스냅샷 amount가 bare `+`로 정본 정확 산출로 입증) → 산술(App.jsx:1335 합계)·표시(344) 무영향·src 무수정. unit_price(integer)는 소수 purchase_price 0건이라 현재 위험 0(별건). dryrun A~L 통과 후 승인 apply(소요 **77ms**·ACCESS EXCLUSIVE 재작성이나 tx962·drugs1114·inv1120 극소형). apply 후 재검증 11항목 통과(3컬럼 numeric·합계 88,402,339/0/106,823,959 보존·정본 **885,285,628.424/106,365,758.469** 무변동·소수 문자열 INSERT 통과·bulk_stock_adjust·0009·마감가드·0055·행수·인덱스7·무잔류). 검증분 전량 ROLLBACK. 코드 무변경(번들 `index-BIA5DGBD.js` 무변동). ★ **롤백은 소수 금액이 실제 저장되기 전에만 무손실**(저장 후 `round()::bigint`로 소수부 소실). ※ 프론트 Math.round는 유지(제거는 별건·전환 후 판단). | `migrate/0079-amount-numeric` |
| 2026-08-27 | 신규. 사용 점검 이력 테이블 | `0080_drug_idle_reviews.sql` — 「사용 점검」 화면용 **이력 누적 테이블 신규 생성**. 참고 모델 drug_change_plans(0038~0040) 복제. 컬럼(8): id(uuid pk)·**tenant_id(uuid NOT NULL·FK tenants·trg_set_tenant_id 자동부여)**·drug_code(text NN·drugs FK 아님·코드 조인)·status(text NN·관찰/중지/보유유지/해제)·reviewed_at(date NN default current_date)·memo·created_by(default auth.uid())·created_at(timestamptz). 인덱스 `(tenant_id,drug_code,reviewed_at desc)`. RLS 4정책(SELECT/INSERT/UPDATE=`tenant_id in current_tenant_ids()`·DELETE=+owner/admin). **[설계 판단 3건·전부 승인]**: ① **UNIQUE 미부여(PK만)** — 모델의 (tenant,code,base_date) 답습 시 (tenant,drug_code,reviewed_at)이 되어 같은 날 재판단 차단→이력 누적 위배. 최신상태=reviewed_at DESC,created_at DESC. ② **status CHECK 미부여** — 모델(plan_status) 자유 select 일관·상태값 확장 유연. ③ **anon GRANT 미부여** — anon은 current_tenant_ids 비어 RLS 전량 차단(기능차 0)·0076 최소권한 개선(모델은 anon CRUD 보유). tenant_id는 모델(nullable)과 달리 **NOT NULL**(트리거가 채운 뒤 NN 검사·orphan 방지). ★ **이 DB는 default privilege가 authenticated 자동부여 안 함 → 신규 테이블마다 `grant select,insert,update,delete ... to authenticated` 명시 필수**(첫 dryrun 42501의 원인). dryrun A~K 통과 후 승인 apply. apply 후 재검증 11항목 통과(구조·RLS4·GRANT auth4/anon0·set_tenant 자동·created_by·이력누적·최신조회·drugs조인·기존정책 무변동[drug_change_plans 4·총 75]·정본 885,285,628.424/106,365,758.469). 검증행 전량 ROLLBACK(운영 0행). 코드 무변경(번들 무변동). 롤백: `drop table if exists public.drug_idle_reviews cascade;`. 3단계(화면) 별건. | `migrate/0080-idle-reviews` |
| 2026-08-28 | 정합. 약효분류명 컬럼 신설 | `0081_drugs_efficacy_class.sql` — **`drugs.efficacy_class text` 추가**(NULL 허용·DEFAULT/CHECK 없음). **배경: 코드는 완비인데 컬럼만 없어 값이 조용히 버려지던 문제.** App.jsx **10곳**(입력칸 3·저장 payload 3·API 매핑 2·폼 상태 2)이 이 필드를 다루나 `drugs`에 컬럼이 없어 PostgREST가 PGRST204(`Could not find the ... column`)를 반환 → **0051 계열 폴백이 키를 조용히 제거하고 재시도** → 사용자에겐 「등록 완료」로 보이나 **값 미저장**. 저장 3경로 = INSERT(App.jsx:575·2512)·UPDATE(599), **셋 다 폴백 보유**라 오류가 표시되지 않았고 흔적은 `console.warn` 한 줄뿐. 표 컬럼(`DRUG_COL_DEFS`)에도 없어 빈 값이 드러나지 않았음. ★ **#243 `additive`·`compound_type`과 같은 유형(세 번째 사례)**. ※ `efficacy_class`는 **마이그레이션 이력에 한 번도 없었음**(만들었다 지운 것이 아니라 처음부터 부재) — `0000_baseline`의 `efficacy`(text DEFAULT '')와는 **별개 컬럼**. **[NULL 허용 근거 4]** ① 기존 1,114건은 「값 없음」이 사실 — `DEFAULT ''`이면 「빈 문자열 입력」과 구분 소멸 ② **코드가 `f.efficacy_class || null`로 빈값을 NULL로 보냄 → NOT NULL이면 빈칸 저장이 23502로 실패** ③ 자유 텍스트(공공API `divNm` 원문)라 CHECK 열거 불가 ④ 0080 status CHECK 미부여와 같은 취지. ※ 기존 `efficacy`의 `DEFAULT ''`는 구 DB 덤프를 물려받은 것이라 **답습하지 않음**. **[backfill 미실행 사유 3]** ① ★ `drug_master.drug_class`는 **숫자 코드**(112·113·214·396…)라 약효분류「명」과 **층위가 다름** — 그대로 넣으면 화면에 `112`가 표시됨 ② **ATC와 99.9% 중복**(매칭 910건 중 909건이 `atc_l2` 보유) ③ 매칭률 81.7%로 부분만 채워짐. 번호→명칭 매핑표가 DB에 없음 → **NULL로 시작하고 신규 등록·수정분부터 API가 채우게 둠**. dryrun A~I + ROLLBACK 전항 통과 후 승인 apply. apply 후 재검증 9항목 통과(컬럼 text·nullable · 저장 3경로 폴백 없이 통과 · 값 저장·재조회 유지 후 **원복 확인** · 기존 NULL 1,114/1,114 · 행수 1,114 · RLS 정책 4 · 컬럼 70→71·인덱스 4 · 정본 **885,285,628.424 / 106,365,758.469** · 검증행 잔류 0). **코드 변경 0줄**(src diff 0 — 컬럼만 생기면 즉시 동작). ★ **[dryrun 교훈] postgres 롤로 INSERT dryrun을 돌리면 `trg_set_tenant_id`가 발동하지 않아 `tenant_id` NOT NULL(0059)에 걸린다** — 실제 앱 경로를 재현하려면 **`set local role authenticated` + `request.jwt.claim.sub` 설정 후 INSERT**할 것(0080 dryrun과 동일 방식). 롤백: `alter table public.drugs drop column if exists efficacy_class;` ※ **값이 저장되기 시작한 뒤에는 그 값이 소실**. | `migrate/0081-efficacy-class` |
| 2026-08-28 | 신규. 명절 병동 약품 신청 | `0083_ward_requests.sql` — **테이블 3개 신설**: `ward_requests`(헤더 9컬럼)·`ward_request_items`(품목 9컬럼·`request_id` FK **ON DELETE CASCADE**)·`ward_request_window`(기간 설정 9컬럼). 0080(drug_idle_reviews) 패턴 복제. **흐름**: 비회원이 분리 앱에서 신청 → Netlify Function이 **service_role로 INSERT** → 약제과 관리 화면(authenticated)에서 조회·수정·사용량 기입. 병동은 저장 후 수정 불가. **[설계 판단 5건]** ① ★ **`tenant_id` NOT NULL 유지 — Function이 명시 지정**. `set_tenant_id_from_user()`는 **`if new.tenant_id is null`일 때만** 채우므로(운영 실측) 명시값을 덮어쓰지 않아 비회원 경로에서도 NN 충족. 트리거는 안전망으로 부착(관리 화면 직접 생성 시 자동 부여). ② **기간 설정은 신규 테이블** — `profiles.settings`는 사용자별이라 Function이 「누구의 settings인지」 정할 수 없고(현재 키 drugCols·favorites·changeCols), 환경변수는 여닫을 때마다 **재배포**가 필요해 요구 불충족. ③ **`season`·`request_year`를 헤더에도 둠**(window와 중복·의도적 비정규화) — window는 「지금 열린 명절」, 헤더는 「이 신청이 어느 명절 것인가」로 **window가 바뀌어도 과거 신청 소속이 유지**돼야 함. ④ **헤더/품목 2테이블** — 1테이블이면 병동·작성자가 품목마다 반복되고 신청 단위 상태(status)·잠금을 표현할 수 없음. ⑤ **`items.drug_code` nullable · `drug_name` NOT NULL** — 기본 동선은 검색 선택이나 「목록에 없는 약」 자유 입력을 나중에 열 때 마이그레이션이 불필요하도록(0080 확장 유연성). **[UNIQUE·CHECK 미부여]** UNIQUE`(tenant,ward,season,year)`는 **같은 병동의 추가 신청을 막으므로** 미부여(중복은 관리 화면 확인). **★ `ward` CHECK 미부여 유지**(사용자 확정) — 병동 신설 가능하고 쓰기 경로가 Function 단일이라 거기서 검증하는 편이 낫다. `season`·`status` CHECK도 같은 취지. **★ `requester_name` NOT NULL 유지**(사용자 확정). ★★ **[dryrun이 잡은 결함] `service_role`도 GRANT가 자동 부여되지 않는다** — 함정 #16(authenticated)의 **확장**. `rolbypassrls=true`라 **RLS는 우회하지만 GRANT는 별개**여서, 최초 초안대로면 **Function 쓰기 경로 전체가 42501로 죽었다**. `grant ... to service_role` 3줄 추가로 해소. ※ 0080에도 service_role GRANT가 없으나 그 테이블은 authenticated만 쓰기 때문에 드러나지 않았을 뿐. **anon GRANT는 부여하지 않음**(Function 경유라 불요·0076 최소권한). RLS 3테이블 on·정책 각 4(품목은 헤더 tenant를 EXISTS 조인으로 따라감). `drug_code`는 drugs FK 아님(코드 기준 조인 규약). dryrun A~J + ROLLBACK 전항 통과 후 승인 apply. apply 후 재검증 12항목 통과(구조 3테이블·컬럼 9/9/9·FK1·인덱스6 / RLS on·정책 4×3 / GRANT authenticated 4×3·service_role 4×3·**anon 0** / service_role tenant 명시 INSERT·**명시값 유지** / authenticated SELECT·UPDATE(usage_qty 1.5) / anon 3테이블 **42501** / drug_code nullable·requester_name NOT NULL / 사용자 CHECK 0건 / **cascade 확인·검증행 잔류 0** / 기존 테이블 35→38·정책 75→87·drugs 1,115 무변동 / 정본 **885,285,628.424 / 106,365,758.469**). ★ **`ward_request_window` 시드 없음(0행)** — 0083 파일에 **INSERT 0건**이라 테이블만 생성됐다. **`is_open=true` 행이 없으므로 신청 기간은 「닫힘」이 기본값**이며, 이는 Function 착수 전 안전한 상태다. 기간을 열려면 관리 화면 또는 별도 시드로 행을 추가해야 한다. 코드 변경 0줄(src diff 0·번들 무변동). 롤백: `drop table if exists public.ward_request_items cascade; drop table if exists public.ward_requests cascade; drop table if exists public.ward_request_window cascade;` ※ 신청이 쌓인 뒤에는 그 데이터가 소실. 3단계(Function 2개)는 별건. | `migrate/0083-ward-requests` |
