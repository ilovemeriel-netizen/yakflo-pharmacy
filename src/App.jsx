...
function Dashboard({ drugs, inv, txns, onNav, onEdit }) {
  ...
  const s = { total: main.length, active: active.length, stopped: drugs.filter(d => d.status === '중지').length, dormant: drugs.filter(d => d.status === '휴면').length, narc: drugs.filter(d => isN(d) && d.status === '사용').length, nonIns: drugs.filter(d => isNonIns(d) && MAIN_STATS.includes(d.status)).length, shortage: main.filter(d => (d.safety_stock || 0) > 0 && (d.current_qty || 0) < d.safety_stock).length, e30: drugs.filter(d => d.expiry_date && d.expiry_date <= fmt(d30) && MAIN_STATS.includes(d.status)).length, e90: drugs.filter(d => d.expiry_date && d.expiry_date > fmt(d30) && d.expiry_date <= fmt(d90) && MAIN_STATS.includes(d.status)).length }
  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
      {[{ l: '비보험', v: s.nonIns, c: t.blue, nav: { menu: 'nonins' } }, { l: '재고부족', v: s.shortage, c: t.red, nav: { menu: 'stock', filter: '부족' }, act: true }, { l: '유효기한 ≤30일', v: s.e30, c: t.red, nav: { menu: 'expiry', focus: 'urgent' }, act: true }, { l: '유효기한 ≤90일', v: s.e90, c: t.amber, nav: { menu: 'expiry', focus: 'warning' }, act: true }].map((c, i) => <div key={i} onClick={() => c.nav && onNav(c.nav)} style={{ background: t.card, borderRadius: 12, padding: '14px 18px', border: `1px solid ${t.border}`, cursor: c.nav ? 'pointer' : 'default', transition: 'all .15s', boxShadow: t.shadow, opacity: (c.act && c.v === 0) ? 0.6 : 1 }} onMouseEnter={hv} onMouseLeave={hx}><div style={{ fontSize: 11, color: t.textM }}>{c.l}</div><div style={{ fontSize: 26, fontWeight: 700, color: (c.act && c.v === 0) ? t.textM : c.c, marginTop: 4 }}>{c.v}</div></div>)}
    </div>
    {(() => { const eN = main.filter(d => { const x = exD(d.expiry_date); return x !== null && x <= 60 }).length; const lN = s.shortage; const nN = main.filter(d => { if (!isN(d)) return false; const x = exD(d.expiry_date); return x !== null && x <= 90 }).length; if (eN + lN + nN === 0) return null; const seg = (label, n, navObj, color) => n > 0 ? <span onClick={ev => { ev.stopPropagation(); onNav(navObj) }} style={{ cursor: 'pointer', textDecoration: 'underline', color, fontWeight: 700 }}>{label} {n}</span> : null; const content = k => <span key={k} aria-hidden={k === 'b' ? 'true' : undefined} style={{ display: 'inline-flex', gap: 18, alignItems: 'center', paddingRight: 56 }}><span aria-hidden="true">⚠</span>{seg('유효기한 60일 이내', eN, { menu: 'expiry', focus: 'urgent' }, t.red)}{seg('재고부족', lN, { menu: 'stock', filter: '부족' }, t.amber)}{seg('향정 임박', nN, { menu: 'alerts' }, t.purple)}</span>; return <div onClick={() => onNav({ menu: 'alerts' })} className="cnc-alert-banner" role="status" style={{ background: t.redL, border: '1px solid ' + t.red + '30', borderRadius: 12, padding: '12px 0', marginBottom: 14, color: t.red, fontSize: 13, fontWeight: 600, cursor: 'pointer', boxShadow: t.shadow, overflow: 'hidden', position: 'relative' }}><div className="cnc-marquee-track" style={{ display: 'inline-flex', whiteSpace: 'nowrap', willChange: 'transform' }}>{content('a')}{content('b')}</div></div>; })()}
