...
function Report({drugs,txns,onNav}){
  ...
  return<div style={{padding:'20px 24px'}}>
    ...
    <div className="cnc-report-table" style={{background:t.card,borderRadius:12,border:`1px solid ${t.border}`,overflow:'hidden'}}>
      <div style={{padding:'12px 18px',borderBottom:`1px solid ${t.border}`,fontWeight:700,fontSize:13,color:t.accent}}>{rtype==='monthly'?`${year}년 ${month}월`:`${year}년 연간`} 보고서 <span title={`표시 ${filtered.length.toLocaleString()}종 (현재 상태·구분 필터 통과) / 전체 ${tableData.length.toLocaleString()}종. 카드·구분별 금액 합계는 표시 약품 기준입니다.`} style={{cursor:'help',borderBottom:`1px dotted ${t.textL}`,fontWeight:600}}>({filtered.length.toLocaleString()} / {tableData.length.toLocaleString()}종, 표시 기준)</span> {ld&&<span style={{fontSize:11,color:t.textL}}>로딩...</span>}</div>
