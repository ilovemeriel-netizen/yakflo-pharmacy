...
function Dashboard({ drugs, inv, txns, onNav, onEdit }) {
  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ fontSize: 11, color: t.textL, margin: "2px 0 8px 2px" }}>구분별 현황 · <b>사용 약품 기준</b> (총 재고수량=수량 합계)</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 18 }}>
      {catData.map(c => { const cc = catC[c.cat] || t.accent; return <div key={c.cat} onClick={() => onNav({ menu: 'druglist', status: ['사용'] })} style={{ background: t.card, borderRadius: 14, padding: '18px 22px', border: `1px solid ${t.border}`, borderLeft: `4px solid ${cc}`, cursor: 'pointer', transition: 'all .15s', boxShadow: t.shadow }} onMouseEnter={hv} onMouseLeave={hx}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12 }}><span style={{ fontSize: 15, fontWeight: 700, color: t.text }}>{c.cat}</span><span style={{ fontSize: 14, fontWeight: 700, color: cc }}>{c.total}개</span></div><div style={{ display: 'flex', gap: 20, alignItems: 'baseline' }}><div><div style={{ fontSize: 10, color: t.textL, marginBottom: 2 }}>총 재고수량</div><div style={{ fontSize: 22, fontWeight: 800, color: t.textM }}>{c.qty.toLocaleString()}</div></div>{c.expSoon > 0 && <div><div style={{ fontSize: 10, color: t.textL, marginBottom: 2 }}>유효기한 주의</div><div style={{ fontSize: 22, fontWeight: 800, color: t.amber }}>{c.expSoon}</div></div>}</div><div style={{ height: 4, background: t.border, borderRadius: 2, marginTop: 12 }}><div style={{ height: '100%', background: cc, borderRadius: 2, width: `${Math.min(c.total / Math.max(s.active, 1) * 100, 100)}%`, opacity: 0.5 }} /></div></div> })}
    </div>
