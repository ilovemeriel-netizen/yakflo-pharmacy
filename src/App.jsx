...
/* ★ 인쇄 1장에 들어가는 표 행수 — 실제 인쇄 후 미세 조정하는 **유일한 손잡이**.
   ...
   [채택 21행]  32.8 + 180.6 = **213.4mm** → 하한과 **정확히 일치 · 여유 0**
       ★ 21은 **상한**이다. 인쇄해서 한 장을 넘기면 **20으로 되돌릴 것**(그다음 19 → 18 → 17).
       ※ 16 → 18 → 19 → 20 → 21로 올려 왔다. 여유가 없으므로 프린터·용지·배율·브라우저
         머리글 설정이 조금만 달라도 2장이 될 수 있다 — 실제 인쇄로만 확인된다(ATC #239 전례).
   ★ 품목이 이 값을 넘으면 빈 행을 채우지 않고 자연스럽게 다음 장으로 넘어간다(thead·tfoot 매 장 반복).
   ★ min-height·vh를 쓰지 않는다(함정 #11) — 행 높이 합으로만 지면을 채운다. */
const WARD_PRINT_ROWS = 21
