...
function AtcDonutsRow({ drugs, t, onPick, sel, onClear }) {
  ...
  const agg = (key) => { const m = {}; drugs.forEach(d => { const v = (d[key] && String(d[key]).trim()) || '미분류'; m[v] = (m[v] || 0) + 1 }); return Object.entries(m).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count) };
  ...
  return <div className="no-print" style={{ background: t.card, borderRadius: 14, border: '1px solid ' + t.border, padding: '14px 18px', marginBottom: 12, boxShadow: t.shadow }}>
    <div style={{ fontSize: 13, fontWeight: 700, color: t.text, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>💊 ATC 단계별 분포 <span style={{ fontSize: 11, fontWeight: 500, color: t.textL }}>· 전체 {total}개 기준</span>{sel ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 9px', borderRadius: 12, background: atcColor(sel.value) + '1A', color: atcColor(sel.value), fontSize: 10, fontWeight: 700, border: '1px solid ' + atcColor(sel.value) + '40' }}>선택: {sel.value || '미분류'}<span onClick={onClear} style={{ cursor: 'pointer', fontWeight: 800 }}>✕</span></span> : null}</div>
    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'flex-start' }}>
      {col('ATC 1단계', '대분류·해부학적', <LDonut data={d1} total={total} onSlice={n => onPick('atc_l1', n === '미분류' ? '' : n)} centerTop={total} centerBot="대분류" t={t} />, d1.map(d => legItem('atc_l1', d)))}
      {col('ATC 2단계', '중분류·치료학적', <LDonut data={d2} total={total} onSlice={n => onPick('atc_l2', n === '미분류' ? '' : n)} centerTop={total} centerBot="중분류" t={t} />, d2.map(d => legItem('atc_l2', d)))}
      {col('ATC 3단계', drill ? '소분류 · 기타 내부' : '소분류·약리학적', <LDonut data={d3data} total={drill ? (d3restSum || 1) : total} onSlice={n => { if (!drill && n === '기타') { setDrill(true) } else { onPick('atc_l3', n === '미분류' ? '' : n) } }} onCenter={drill ? () => setDrill(false) : null} centerTop={drill ? '기타' : total} centerBot={drill ? '▸뒤로' : '소분류'} t={t} />, d3data.map(d => d.name === '기타' ? <div key="기타" onClick={() => setDrill(true)} title="기타 펼치기(드릴다운)" style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', padding: '2px 5px', borderRadius: 6 }} onMouseEnter={e => e.currentTarget.style.background = t.bg} onMouseLeave={e => e.currentTarget.style.background = ''}><span style={{ width: 9, height: 9, borderRadius: 3, background: atcColor('기타'), flexShrink: 0 }} /><span style={{ fontSize: 10, color: t.purple, flex: 1, fontWeight: 700 }}>기타(상위12 외) ▸</span><span style={{ fontSize: 10, fontWeight: 700, color: t.text }}>{d.count}</span></div> : legItem('atc_l3', d)))}
    </div>
