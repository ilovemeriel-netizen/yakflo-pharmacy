...
function AtcTable({ t, cols, rows, minWidth, colWidths, resetable }) {
  ...
  const hf = {};
  cols.forEach(c => { if (c.filter) hf[c.k] = { items: _u(rows.map(r => r[c.k])).map(String).sort((a, b) => a.localeCompare(b, 'ko')), value: filters[c.k] != null ? filters[c.k] : null, on: v => { setFilters(f => { const n = { ...f }; if (v) n[c.k] = v; else delete n[c.k]; return n; }); setPage(1); } }; });
  let list = rows.filter(r => Object.keys(filters).every(k => String(r[k]) === String(filters[k])));
  list = so(list);
...
function AtcView({ drugs, onReload }) {
  ...
  return <div>
    ...
    <div className="no-print" style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 12 }}>
      ...
      {canEdit && <button onClick={() => setManageModal(true)} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid ' + t.amber, background: t.card, color: t.amber, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>수기 조제 관리{manualEx.length ? ' (' + manualEx.length + ')' : ''}</button>}
    ...
    {viewMode === 'proposal' && <div className="no-print" style={{ background: t.card, border: '1px solid ' + t.border, borderRadius: 12, padding: 14, marginBottom: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
        {[{ tag: 'A', tagBg: t.greenL, tagFg: t.green, title: '신규 진입', note: '카세트 확보 필요', rows: [...aNew.map(d => ({ _slot: atcSlotKey(propSlotOf[d.drug_code]), slotLabel: propSlotOf[d.drug_code], name: d.drug_name, r3: usageScore(d), rank: rankFull.get(d.drug_code) || null, excl: false, reason: null })), ...manualEx.filter(d => (rankFull.get(d.drug_code) || 999) <= 88 && !curCodes2.has(d.drug_code)).map(d => ({ _slot: 9999, slotLabel: null, name: d.drug_name, r3: usageScore(d), rank: rankFull.get(d.drug_code) || null, excl: true, reason: d.atc_exclude_reason }))] }, { tag: 'B', tagBg: t.amberL, tagFg: t.amber, title: '제외', note: '수동 조제 전환', rows: bOut.map(cc => { const dd = (drugs || []).find(x => x.drug_code === cc); return { _slot: atcSlotKey(dd && dd.atc_slot), slotLabel: dd && dd.atc_slot, name: (dd && dd.drug_name) || cc, r3: dd ? usageScore(dd) : 0, rank: rankFull.get(cc) || null, excl: false, reason: null }; }) }].map(sec => <div key={sec.tag} style={{ border: '1px solid ' + t.border, borderRadius: 10, overflow: 'hidden' }}>
          ...
          <div style={{ padding: 8 }}><AtcTable t={t} minWidth={340} colWidths={[68, 148, 60, 56]} resetable rows={sec.rows} cols={[{ k: '_slot', h: '슬롯', align: 'center', filter: true, render: r => r.excl ? B('수기 조제', t.amberL, t.amber, t.amber) : (r.slotLabel ? B(r.slotLabel, t.accentL, t.accent, t.accent) : <span style={{ color: t.textL }}>-</span>) }, { k: 'name', h: '약품명', align: 'left', render: r => r.excl ? <span>{r.name} <span style={{ color: t.textL, fontSize: 10 }}>({r.reason || '수기'})</span></span> : r.name }, { k: 'r3', h: '3개월', align: 'right' }, { k: 'rank', h: '순위', align: 'right', render: r => r.rank == null ? '-' : r.rank }]} /></div>
        </div>)}
      </div>
      <div style={{ marginTop: 12, border: '1px solid ' + t.border, borderRadius: 10, overflow: 'hidden' }}>
        ...
        <div style={{ padding: 8 }}><AtcTable t={t} minWidth={420} colWidths={[56, 170, 64, 130]} resetable rows={yebi12.map(d => ({ rank: rankFull.get(d.drug_code) || null, name: d.drug_name, r3: usageScore(d), _u: usageScore(d) }))} cols={[{ k: 'rank', h: '순위', align: 'center', render: r => r.rank == null ? '-' : r.rank }, { k: 'name', h: '약품명', align: 'left' }, { k: 'r3', h: '3개월', align: 'right' }, { k: '_u', h: '88위 경계 대비', align: 'left', render: r => { const bnd = usageScore(assignable[87]) || 1; const w = Math.max(2, Math.min(100, r._u / bnd * 100)); return <div style={{ height: 6, borderRadius: 3, background: t.lavender, width: w + '%' }} />; } }]} /></div>
      </div>
      <div style={{ marginTop: 12, border: '1px solid ' + t.border, borderRadius: 10, overflow: 'hidden' }}>
