...
function DrugList({ drugs, navFilter: nf, onEdit, onReload, nonins }) {
  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    : <AtcDonutsRow drugs={drugs} t={t} nonins={nonins} sel={donutF} onPick={(level, value) => { setDonutF({ level, value }); setPage(1) }} onClear={() => { setDonutF(null); setPage(1) }} onCenterReset={_resetF} />}
    {/* ★ cnc-no-anchor — 이 카드(요약바+표+페이지네이션)를 스크롤 앵커 후보에서 제외한다.
        검색·필터로 행 수가 바뀔 때 브라우저가 스크롤을 보정하며 클릭 좌표를 흔드는 것을 막는다.
        근거·계측치는 index.css 의 .cnc-no-anchor 주석 참조. */}
    <div className="cnc-no-anchor" style={{ background: t.card, borderRadius: 14, border: `1px solid ${t.border}`, overflow: 'visible', boxShadow: t.shadow }}>
