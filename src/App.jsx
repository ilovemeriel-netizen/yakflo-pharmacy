...
function AtcView({ drugs, onReload }) {
  ...
  const [manageDrag, setManageDrag] = useState({ x: 0, y: 0 });
  const [manageDragging, setManageDragging] = useState(false);
  const manageBoxRef = useRef(null); const manageStart = useRef(null);
  ...
  function manageMove(e) { const s = manageStart.current; if (!s) return; let nx = s.ox + (e.clientX - s.mx), ny = s.oy + (e.clientY - s.my); const minX = 40 - s.w - s.natLeft, maxX = window.innerWidth - 40 - s.natLeft; const minY = 0 - s.natTop, maxY = window.innerHeight - 40 - s.natTop; nx = Math.max(minX, Math.min(maxX, nx)); ny = Math.max(minY, Math.min(maxY, ny)); setManageDrag({ x: nx, y: ny }); }
  function manageUp() { setManageDragging(false); manageStart.current = null; window.removeEventListener('mousemove', manageMove); window.removeEventListener('mouseup', manageUp); }
  function manageDown(e) { if (e.button !== 0) return; if (e.target && e.target.closest && e.target.closest('button, input, select, textarea, a, table')) return; const box = manageBoxRef.current; if (!box) return; const r = box.getBoundingClientRect(); manageStart.current = { mx: e.clientX, my: e.clientY, ox: manageDrag.x, oy: manageDrag.y, natLeft: r.left - manageDrag.x, natTop: r.top - manageDrag.y, w: r.width, h: r.height }; setManageDragging(true); window.addEventListener('mousemove', manageMove); window.addEventListener('mouseup', manageUp); e.preventDefault(); }
  ...
  return <div>
    ...
    <div className="no-print" style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 12 }}>
      ...
      {canEdit && <button onClick={() => { setManageDrag({ x: 0, y: 0 }); setManageModal(true); }} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid ' + t.amber, background: t.card, color: t.amber, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>수기 조제 관리{manualEx.length ? ' (' + manualEx.length + ')' : ''}</button>}
    ...
    <div className="atc-print-only">
      {[0, 1].map(pg => { const rows = pg === 0 ? slot91Ga.slice(0, 46) : slot91Ga.slice(46); return <div key={pg} className="atc-page">
        ...
        <table><thead><tr>{[['슬롯', '12%', 'center'], ['약품명', '36%', 'left'], ['성분명', '40%', 'left'], ['배지', '12%', 'center']].map(([h, w, al]) => <th key={h} style={{ fontSize: '8pt', textAlign: al, borderBottom: '0.5pt solid ' + t.border, width: w }}>{h}</th>)}</tr></thead>
        <tbody>{rows.map(({ slot, d }, i) => <tr key={i}><td className="sl" style={{ fontWeight: 700, borderBottom: '0.3pt solid ' + t.border }}>{slot}</td><td className="nm" style={{ fontWeight: 600, borderBottom: '0.3pt solid ' + t.border }} title={d.drug_name}>{d.drug_name}</td><td className="ig" style={{ color: t.textM, borderBottom: '0.3pt solid ' + t.border }} title={d.ingredient_kr || ''}>{d.ingredient_kr || ''}</td><td className="bd" style={{ borderBottom: '0.3pt solid ' + t.border }}>{dBadges(d).map((b, bi) => <span key={bi} style={badgeStyle(b.bg, b.fg, b.bd)}>{b.x}</span>)}</td></tr>)}</tbody></table>
