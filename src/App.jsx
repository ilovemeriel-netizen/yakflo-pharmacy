...
function StockStatus({drugs,inv,navFilter:nf,onEdit,onAdjust,onReload,onDispose}){
  ...
  const sg={전체:merged.length,긴급:merged.filter(d=>d.stockStatus==='긴급').length,주문필요:merged.filter(d=>d.stockStatus==='주문필요').length,재고없음:merged.filter(d=>d.stockStatus==='재고없음').length,정상:merged.filter(d=>d.stockStatus==='정상').length,과잉:merged.filter(d=>d.stockStatus==='과잉').length}
