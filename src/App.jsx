...
/* 유효기한 밴드 경계(일) — 단일 정의: 만료≤0 · 긴급≤30 · 주의31~90 · 확인91~180 · 정상>180. 향정마약 특례 180·미사용 365는 별도. */
const EXP_BAND = { expired: 0, urgent: 30, warning: 90, notice: 180 }
...
function Dashboard({ drugs, inv, txns, onNav, onEdit }) {
  ...
  const s = { total: main.length, active: active.length, stopped: drugs.filter(d => d.status === '중지').length, dormant: drugs.filter(d => d.status === '휴면').length, narc: drugs.filter(d => isN(d) && d.status === '사용').length, nonIns: drugs.filter(d => isNonIns(d) && MAIN_STATS.includes(d.status)).length, shortage: main.filter(d => stockStat(d) === '긴급').length, reorder: main.filter(d => stockStat(d) === '주문필요').length, e30: drugs.filter(d => { const x = exD(d.expiry_date); return x !== null && x <= EXP_BAND.urgent && MAIN_STATS.includes(d.status) }).length, e90: drugs.filter(d => { const x = exD(d.expiry_date); return x !== null && x > EXP_BAND.urgent && x <= EXP_BAND.warning && MAIN_STATS.includes(d.status) }).length }
...
function ExpiryAlert({drugs,onEdit,focusLevel,onReload,onDispose}){
  ...
  const alertSt=days=>{if(days===null)return{text:'',c:t.textL,bg:''};if(days<=EXP_BAND.expired)return{text:'★만료★',c:'#fff',bg:t.red};if(days<=EXP_BAND.urgent)return{text:'▲긴급▲',c:'#fff',bg:t.amber};if(days<=EXP_BAND.warning)return{text:'◆주의◆',c:t.amber,bg:t.amberL};if(days<=EXP_BAND.notice)return{text:'●확인●',c:'#fff',bg:t.blue};return{text:'정상',c:t.green,bg:''}}
  const g={urgent:fd.filter(d=>{const x=exD(d.expiry_date);return x!==null&&x<=EXP_BAND.urgent&&(d.current_qty||0)>0}),warning:fd.filter(d=>{const x=exD(d.expiry_date);return x!==null&&x>EXP_BAND.urgent&&x<=EXP_BAND.warning&&(d.current_qty||0)>0}),notice:fd.filter(d=>{const x=exD(d.expiry_date);return x!==null&&x>EXP_BAND.warning&&x<=EXP_BAND.notice&&(d.current_qty||0)>0}),narcotic:drugs.filter(d=>{const x=exD(d.expiry_date);return x!==null&&x<=180&&isN(d)&&cats.includes(d.category)}),unused:fd.filter(d=>isUnused(d)&&(d.current_qty||0)>0)}
  ...
  const lvs=[{k:'urgent',l:'긴급',sub:'≤30일',c:t.red},{k:'warning',l:'주의',sub:'31~90일',c:t.amber},{k:'notice',l:'확인',sub:'91~180일',c:t.blue},{k:'narcotic',l:'향정마약',sub:'≤180일',c:t.purple},{k:'unused',l:'미사용',sub:'1년 이상',c:t.red}]
