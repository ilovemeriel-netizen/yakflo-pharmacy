...
function AtcView({ drugs, onReload }) {
  ...
  const [excludeReason, setExcludeReason] = useState('수기 조제 필요');
  const EX_REASONS = ['수기 조제 필요', '분할 조제', '규격/카세트 없음', '저빈도', '기타'];
  ...
  async function saveExclude() { if (!excludeSel) return; const { error } = await supabase.from('drugs').update({ atc_excluded: true, atc_exclude_reason: excludeReason || null }).eq('drug_code', excludeSel.drug_code); if (error) { setMsg('수기 제외 실패: ' + error.message); setTimeout(() => setMsg(null), 4000); return; } setMsg('수기 조제 제외 추가 — ' + excludeSel.drug_name); setExcludeModal(false); setExcludeSel(null); setExcludeQ(''); onReload && onReload(); setTimeout(() => setMsg(null), 3500); }
  ...
  return <div>
    ...
    {excludeModal && <div className="no-print" onClick={() => { setExcludeModal(false); setExcludeSel(null); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}><div onClick={ev => ev.stopPropagation()} style={{ background: t.card, borderRadius: 12, border: '1px solid ' + t.border, padding: 20, maxWidth: 460, width: '100%' }}><div style={{ fontSize: 14, fontWeight: 700, color: t.text, marginBottom: 8 }}>수기 조제 — 배정 제외 약품 추가</div><input value={excludeQ} onChange={e => setExcludeQ(e.target.value)} placeholder="약품코드·약품명·성분명 검색" style={{ width: '100%', padding: '8px 12px', border: '1px solid ' + t.border, borderRadius: 8, fontSize: 12, boxSizing: 'border-box', background: t.bg, color: t.text, outline: 'none' }} /><div style={{ maxHeight: '36vh', overflowY: 'auto', marginTop: 8, border: '1px solid ' + t.border, borderRadius: 8 }}>{(drugs || []).filter(d => d.category === '경구제' && d.status === '사용' && isSolid(d) && !d.atc_excluded).filter(d => { const kw = excludeQ.trim().toLowerCase(); return !kw || String(d.drug_code || '').toLowerCase().includes(kw) || String(d.drug_name || '').toLowerCase().includes(kw) || String(d.ingredient_kr || '').toLowerCase().includes(kw); }).slice(0, 50).map((d, i) => <div key={i} onClick={() => setExcludeSel(d)} style={{ padding: '6px 10px', borderBottom: '1px solid ' + t.border, cursor: 'pointer', fontSize: 12, background: excludeSel && excludeSel.drug_code === d.drug_code ? t.accentL : '' }}><b style={{ color: t.accent }}>{d.drug_name}</b> <span style={{ color: t.textL, fontSize: 10 }}>{d.drug_code}{d.atc_slot ? ' · 현재 ' + d.atc_slot : ''}</span></div>)}</div>{excludeSel && <div style={{ marginTop: 10, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}><span style={{ fontSize: 12, fontWeight: 700, color: t.accent }}>{excludeSel.drug_name}</span><span style={{ fontSize: 11, color: t.textM }}>사유</span><select value={excludeReason} onChange={e => setExcludeReason(e.target.value)} style={{ padding: '6px 10px', border: '1px solid ' + t.border, borderRadius: 8, fontSize: 12, background: t.bg, color: t.text, outline: 'none' }}>{EX_REASONS.map(r => <option key={r} value={r}>{r}</option>)}</select></div>}<div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}><button onClick={() => { setExcludeModal(false); setExcludeSel(null); }} style={{ padding: '8px 14px', borderRadius: 8, border: '1px solid ' + t.border, background: 'transparent', color: t.textM, cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>취소</button><button onClick={saveExclude} disabled={!excludeSel} style={{ padding: '8px 14px', borderRadius: 8, border: '1px solid ' + (excludeSel ? t.accent : t.border), background: excludeSel ? t.accent : t.bg, color: excludeSel ? t.navText : t.textL, cursor: excludeSel ? 'pointer' : 'default', fontSize: 12, fontWeight: 700 }}>저장</button></div></div></div>}
    ...
    {viewMode === 'proposal' && <div className="no-print" style={{ background: t.card, border: '1px solid ' + t.border, borderRadius: 12, padding: 14, marginBottom: 14 }}>
      ...
      <div style={{ marginTop: 12, background: t.card, border: '1px solid ' + t.border, borderRadius: 10, padding: '10px 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}><span style={{ fontSize: 11.5, fontWeight: 700, color: t.text }}>수기 조제 (배정 제외) 관리</span>{B('수기 조제', t.amberL, t.amber, t.amber)}<span style={{ fontSize: 10, fontWeight: 500, color: t.textM }}>순위 산정엔 포함, 슬롯 미배정 · 현재 {manualEx.length}건</span><div style={{ flex: 1 }} />{canEdit && <button onClick={() => { setExcludeModal(true); setExcludeSel(null); setExcludeQ(''); }} style={{ padding: '4px 12px', borderRadius: 8, border: '1px solid ' + t.accent, background: t.accentL, color: t.accent, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>+ 약품 추가</button>}</div>
        {manualEx.length ? <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>{[...manualEx].sort((a, b) => String(a.drug_name || '').localeCompare(String(b.drug_name || ''), 'ko')).map((d, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, flexWrap: 'wrap' }}><span style={{ fontWeight: 600, color: t.text }}>{d.drug_name}</span><span style={{ color: t.textL, fontSize: 10 }}>{d.drug_code} · {rankFull.get(d.drug_code) || '-'}위</span>{d.atc_exclude_reason && B(d.atc_exclude_reason, t.amberL, t.amber, t.amber)}<div style={{ flex: 1 }} />{canEdit && <button onClick={() => unExclude(d)} style={{ padding: '2px 8px', borderRadius: 6, border: '1px solid ' + t.border, background: 'transparent', color: t.textM, cursor: 'pointer', fontSize: 10, fontWeight: 700 }}>해제</button>}</div>)}</div> : <div style={{ fontSize: 10.5, color: t.textL }}>수기 조제로 지정된 약품이 없습니다.</div>}
      </div>
