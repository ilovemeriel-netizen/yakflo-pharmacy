...
function Ordering({ drugs, onAdjust }) {
  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>{(() => { const sc = {}; cand.forEach(d => { const k = effSup(d); sc[k] = (sc[k] || 0) + 1 }); const chips = [{ k: null, l: '전체', n: cand.length }, ...suppliersSorted.map(su => ({ k: su.name, l: su.name, n: sc[su.id] || 0 })), { k: '미지정', l: '미지정', n: sc['__none'] || 0 }]; return chips.map(ch => { const on = supFilter === ch.k; return <button key={ch.l} onClick={() => setSupFilter(ch.k)} style={{ padding: '5px 12px', borderRadius: 8, border: '1px solid ' + (on ? t.accent : t.border), background: on ? t.accentL : t.card, color: on ? t.accent : t.textM, cursor: 'pointer', fontSize: 11, fontWeight: on ? 700 : 500, whiteSpace: 'nowrap' }}>{ch.l} {ch.n}</button> }) })()}</div>
    ...
    {supFilter && supFilter !== '미지정' && (() => { const _supId = (suppliers.find(su => su.name === supFilter) || {}).id; if (!_supId) return null; const _list = drugs.filter(d => effSup(d) === _supId); const _rop = d => { const sf = Number(d.safety_stock) || 0; return sf <= 0 ? null : Math.max(Math.ceil(sf + ((Number(d.monthly_avg) || 0) / 30) * LEAD_TIME), 1); }; const _sc = st => (st === '긴급' || st === '재고없음') ? t.red : st === '주문필요' ? t.amber : st === '기준미설정' ? t.textL : t.green; return <div style={{ marginTop: 20 }}>
      ...
      {!_list.length ? <div style={{ background: t.card, borderRadius: 12, border: '1px solid ' + t.border, padding: 24, textAlign: 'center', color: t.textL }}>등록된 약품이 없습니다. 약품 추가로 등록하세요.</div> : <div style={{ background: t.card, borderRadius: 12, border: '1px solid ' + t.border, overflow: 'hidden', boxShadow: t.shadow }}><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}><thead><tr>{['약품명', '월평균', '안전재고', '발주점', '현재고', '상태', '제거'].map((h, hi) => <th key={h} style={{ textAlign: hi === 0 ? 'left' : hi === 6 ? 'center' : 'right', padding: '8px 14px', color: t.textM, borderBottom: '1px solid ' + t.border, fontSize: 11, whiteSpace: 'nowrap' }}>{h}</th>)}</tr></thead><tbody>{_list.map((d, i) => { const _st = stockStat(d); const _r = _rop(d); return <tr key={i} style={{ borderBottom: '1px solid ' + t.border }}><td style={{ padding: '7px 14px', textAlign: 'left' }}><span onClick={() => open360 && open360(d)} style={{ color: t.accent, fontWeight: 600, cursor: 'pointer' }}>{d.drug_name}</span> <span style={{ color: t.textL, fontSize: 10 }}>{d.drug_code}</span></td><td style={{ padding: '7px 14px', textAlign: 'right', color: t.textM }}>{(Number(d.monthly_avg) || 0).toLocaleString()}</td><td style={{ padding: '7px 14px', textAlign: 'right', color: t.textM }}>{(Number(d.safety_stock) || 0).toLocaleString()}</td><td style={{ padding: '7px 14px', textAlign: 'right', color: t.textM }}>{_r == null ? '-' : _r.toLocaleString()}</td><td style={{ padding: '7px 14px', textAlign: 'right', fontWeight: 600 }}>{(Number(d.current_qty) || 0).toLocaleString()}</td><td style={{ padding: '7px 14px', textAlign: 'right' }}><Bd bg={_sc(_st) + '18'} color={_sc(_st)}>{_st}</Bd></td><td style={{ padding: '7px 14px', textAlign: 'center' }}><button onClick={() => assignSupplier(d.drug_code, '')} title="목록에서 제거(약품 유지)" style={{ padding: '2px 9px', borderRadius: 6, border: '1px solid ' + t.red, background: 'transparent', color: t.red, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>×</button></td></tr> })}</tbody></table></div></div>}
    </div>; })()}
