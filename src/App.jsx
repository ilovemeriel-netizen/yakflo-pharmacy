...
function DrugList({ drugs, navFilter: nf, onEdit, nonins }) {
  ...
  function _resetF(){setSearch('');setCats(CATS);setStats(['사용']);setNarcOnly(false);setHanoeOnly(false);setInsF('전체');setRxF(null);setDonutF(null);setCmpHF(null);setStoHF(null);setLocHF(null);setAtcF(null);setSort('drug_name','asc');setPage(1)}
  const _df=(search.trim()?1:0)+(cats.length!==CATS.length?1:0)+(!(stats.length===1&&stats[0]==='사용')?1:0)+(narcOnly?1:0)+(hanoeOnly?1:0)+(insF!=='전체'?1:0)+(rxF?1:0)+(donutF?1:0)+(cmpHF?1:0)+(stoHF?1:0)+(locHF?1:0)+(atcF?1:0)+((sk!=='drug_name'||sd!=='asc')?1:0);const _showReset=_df>0;
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ background: t.card, borderRadius: 14, border: `1px solid ${t.border}`, overflow: 'hidden', boxShadow: t.shadow }}>
      ...
      <StandardTable t={t} TS={TS} sk={sk} sd={sd} setSort={(k,sdv)=>{setSort(k,sdv);setPage(1)}} hf={hf} hscroll={{noLabel:true,ends:true}} cols={COLS.map(([k,h])=>({k,h,th:{whiteSpace:'nowrap',textAlign:(k==='current_qty'||k==='purchase_price')?'right':'left',...(k==='drug_name'?{minWidth:220}:{})},...(k==='drug_code'?{sticky:{left:0,w:128}}:k==='drug_name'?{sticky:{left:128}}:{})}))}>
        <tbody>{!paged.length ? <tr><td colSpan={15} style={{ padding: 40, textAlign: 'center', color: t.textL }}>검색 결과 없음</td></tr> : paged.map((d, i) => { const acc = atcColor(d.atc_l1); const atcChips = [['atc_l1', d.atc_l1], ['atc_l2', d.atc_l2], ['atc_l3', d.atc_l3]].filter(c => c[1] && String(c[1]).trim()); const sm = d.storage_method || ''; const cold = sm.includes('냉장'); const shade = sm.includes('차광'); const smBg = cold ? t.blueL : shade ? t.amberL : t.bg; const smFg = cold ? t.blue : shade ? t.amber : t.textM; const cmp = d.compound_type || ''; return <tr key={i} style={{ borderBottom: '1px solid ' + t.border, background: i % 2 ? t.bg : '' }} onMouseEnter={e => { const r = e.currentTarget; const hb = i % 2 ? t.card : t.glass; r.style.background = hb; const c = r.children; if (c[0]) c[0].style.background = hb; if (c[1]) c[1].style.background = hb }} onMouseLeave={e => { const r = e.currentTarget; const z = i % 2 ? t.bg : ''; const sz = i % 2 ? t.bg : t.card; r.style.background = z; const c = r.children; if (c[0]) c[0].style.background = sz; if (c[1]) c[1].style.background = sz }}>
          <td style={{ padding: '9px 12px', fontSize: 10, color: t.textM, textAlign: 'left', whiteSpace: 'nowrap', position: 'sticky', left: 0, zIndex: 2, minWidth: 128, maxWidth: 128, width: 128, overflow: 'hidden', background: i % 2 ? t.bg : t.card }}><span onClick={() => open360 && open360(d)} title="360° 상세 보기" style={{ color: t.accent, cursor: 'pointer', borderBottom: '1px dotted ' + t.textL }}>{d.drug_code}</span><NT d={d} /></td>
          <td style={{ padding: '8px 12px', fontWeight: 600, textAlign: 'left', color: t.accent, cursor: 'pointer', minWidth: 220, maxWidth: 280, position: 'sticky', left: 128, zIndex: 2, background: i % 2 ? t.bg : t.card }} onClick={() => onEdit(d)} onMouseEnter={e => { e.currentTarget.style.color = t.purple }} onMouseLeave={e => { e.currentTarget.style.color = t.accent }} title={d.drug_name || ''}><span style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: 1.3 }}>{d.drug_name}</span></td>
          <td style={{ padding: '9px 12px', textAlign: 'left', color: t.textM, fontSize: 11, whiteSpace: 'nowrap' }}>{d.category || '-'}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left', color: t.textL, fontSize: 10, fontStyle: 'italic', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={d.ingredient_en || ''}>{d.ingredient_en || '-'}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left', color: t.textM, fontSize: 11, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={d.ingredient_kr || ''}>{d.ingredient_kr || '-'}</td>
