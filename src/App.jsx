    ...
    <div style={{ background: t.cardSolid, borderRadius: 16, width: '100%', maxWidth: 760, maxHeight: '92vh', overflowY: 'auto', border: `1px solid ${t.border}`, boxShadow: t.shadowH, transform: `translate(${pos.x}px, ${pos.y}px)` }} onClick={e => e.stopPropagation()}>
      ...
      <div style={{ padding: '16px 24px 20px' }}>
        ...
        {lookupInfo&&<div style={{marginBottom:12,background:t.bg,borderRadius:10,border:`1px solid ${t.purple}40`,padding:'14px 18px'}}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}><div><label style={lb}>구입단가</label><input type="number" value={f.purchase_price} onChange={e => set('purchase_price', e.target.value)} style={ip} /></div><div><label style={lb}>보험코드</label><input value={f.insurance_code} onChange={e => set('insurance_code', e.target.value)} style={ip} /></div></div>
...
/* ═══ 대시보드 — Bento Grid ═══ */
function HScroll({ children, noLabel, ends }) {
  const { t } = useTheme();
  const ref = useRef(null);
...
function AtcDonutsRow({ drugs, t, onPick, sel, onClear, nonins }) {
  ...
  return <div className="no-print" style={{ background: t.card, borderRadius: 14, border: '1px solid ' + t.border, padding: '14px 18px', marginBottom: 12, boxShadow: t.shadow }}>
    ...
    <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', overflowX: 'auto' }}>
      {col('1단계', '대분류·해부학적', <LDonut data={d1} total={total} onSlice={n => onPick('atc_l1', n === '미분류' ? '' : n)} centerTop={total} centerBot="대분류" t={t} />, d1.map(d => legItem('atc_l1', d)))}
      {col('2단계', '중분류·치료학적', <LDonut data={d2} total={total} onSlice={n => onPick('atc_l2', n === '미분류' ? '' : n)} centerTop={total} centerBot="중분류" t={t} />, d2.map(d => legItem('atc_l2', d)))}
      {col('3단계', drill ? '소분류 · 기타 내부' : '소분류·약리학적', <LDonut data={d3data} total={drill ? (d3restSum || 1) : total} onSlice={n => { if (!drill && n === '기타') { setDrill(true) } else { onPick('atc_l3', n === '미분류' ? '' : n) } }} onCenter={drill ? () => setDrill(false) : null} centerTop={drill ? '기타' : total} centerBot={drill ? '▸뒤로' : '소분류'} t={t} />, d3data.map(d => d.name === '기타' ? <div key="기타" onClick={() => setDrill(true)} title="기타 펼치기(드릴다운)" style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', padding: '2px 5px', borderRadius: 6 }} onMouseEnter={e => e.currentTarget.style.background = t.bg} onMouseLeave={e => e.currentTarget.style.background = ''}><span style={{ width: 9, height: 9, borderRadius: 3, background: atcColor('기타'), flexShrink: 0 }} /><span style={{ fontSize: 10, color: t.purple, flex: 1, textAlign: 'left', fontWeight: 700 }}>기타(상위12 외) ▸</span><span style={{ fontSize: 10, fontWeight: 700, color: t.text }}>{d.count}</span></div> : legItem('atc_l3', d)))}
    </div>
...
function DrugList({ drugs, navFilter: nf, onEdit, nonins }) {
  ...
  const COLS = [['drug_code', '약품코드'], ['drug_name', '약품명'], ['category', '구분'], ['ingredient_en', '성분(EN)'], ['ingredient_kr', '성분(KR)'], ['atc_l1', 'ATC'], ['compound_type', '복합/단일'], ['manufacturer', '제조사'], ['purchase_price', '개당단가'], ['current_qty', '현재고'], ['insurance_type', '급여'], ['storage_method', '보관'], ['storage_location', '위치'], ['expiry_date', '유효기한'], ['status', '상태']]
  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ background: t.card, borderRadius: 14, border: `1px solid ${t.border}`, overflow: 'hidden', boxShadow: t.shadow }}>
      ...
      <StandardTable t={t} TS={TS} sk={sk} sd={sd} setSort={(k,sdv)=>{setSort(k,sdv);setPage(1)}} hf={hf} cols={COLS.map(([k,h])=>({k,h,th:{whiteSpace:'nowrap',textAlign:(k==='current_qty'||k==='purchase_price')?'right':'left',...(k==='drug_name'?{minWidth:220}:{}),...((k==='drug_code'||k==='drug_name')?{borderRight:'1px solid '+t.border}:{})},...(k==='drug_code'?{sticky:{left:0,w:128}}:k==='drug_name'?{sticky:{left:128}}:{})}))}>
        <tbody>{!paged.length ? <tr><td colSpan={15} style={{ padding: 40, textAlign: 'center', color: t.textL }}>검색 결과 없음</td></tr> : paged.map((d, i) => { const acc = atcColor(d.atc_l1); const atcChips = [['atc_l1', d.atc_l1], ['atc_l2', d.atc_l2], ['atc_l3', d.atc_l3]].filter(c => c[1] && String(c[1]).trim()); const sm = d.storage_method || ''; const cold = sm.includes('냉장'); const shade = sm.includes('차광'); const smBg = cold ? t.blueL : shade ? t.amberL : t.bg; const smFg = cold ? t.blue : shade ? t.amber : t.textM; const cmp = d.compound_type || ''; return <tr key={i} style={{ borderBottom: '1px solid ' + t.border, background: i % 2 ? t.bg : '' }} onMouseEnter={e => e.currentTarget.style.background = t.glass} onMouseLeave={e => e.currentTarget.style.background = i % 2 ? t.bg : ''}>
          <td style={{ padding: '9px 12px', fontSize: 10, color: t.textM, textAlign: 'left', whiteSpace: 'nowrap', position: 'sticky', left: 0, zIndex: 2, minWidth: 128, maxWidth: 128, width: 128, overflow: 'hidden', background: i % 2 ? t.bg : t.card, borderRight: '1px solid ' + t.border }}><span onClick={() => open360 && open360(d)} title="360° 상세 보기" style={{ color: t.accent, cursor: 'pointer', borderBottom: '1px dotted ' + t.textL }}>{d.drug_code}</span><NT d={d} /></td>
          <td style={{ padding: '8px 12px', fontWeight: 600, textAlign: 'left', color: t.accent, cursor: 'pointer', minWidth: 220, maxWidth: 280, position: 'sticky', left: 128, zIndex: 2, background: i % 2 ? t.bg : t.card, borderRight: '1px solid ' + t.border }} onClick={() => onEdit(d)} onMouseEnter={e => { e.currentTarget.style.color = t.purple }} onMouseLeave={e => { e.currentTarget.style.color = t.accent }} title={d.drug_name || ''}><span style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: 1.3 }}>{d.drug_name}</span></td>
          <td style={{ padding: '9px 12px', textAlign: 'left', color: t.textM, fontSize: 11, whiteSpace: 'nowrap' }}>{d.category || '-'}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left', color: t.textL, fontSize: 10, fontStyle: 'italic', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={d.ingredient_en || ''}>{d.ingredient_en || '-'}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left', color: t.textM, fontSize: 11, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={d.ingredient_kr || ''}>{d.ingredient_kr || '-'}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left' }}>{atcChips.length ? <span style={{ display: 'inline-flex', gap: 3, flexWrap: 'wrap' }}>{atcChips.map((c, j) => { const v = c[1]; return <span key={j} onClick={() => { setDonutF({ level: c[0], value: v }); setPage(1) }} title={'이 목록을 ' + v + ' 로 필터'} style={{ display: 'inline-block', padding: '2px 7px', borderRadius: 8, fontSize: 9, fontWeight: 700, background: acc + (j === 0 ? '22' : '12'), color: acc, border: '1px solid ' + acc + '33', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', cursor: 'pointer' }}>{v}</span> })}</span> : <span style={{ color: t.textL, fontSize: 10 }}>-</span>}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left' }}>{cmp ? <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 600, background: cmp === '복합제' ? t.purpleL : t.bg, color: cmp === '복합제' ? t.purple : t.textM, border: '1px solid ' + (cmp === '복합제' ? t.purple : t.textL) + '33', whiteSpace: 'nowrap' }}>{cmp}</span> : <span style={{ color: t.textL, fontSize: 10 }}>-</span>}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left', color: t.textM, fontSize: 11, maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={d.manufacturer || ''}>{d.manufacturer || '-'}</td>
          <td style={{ padding: '9px 12px', textAlign: 'right', color: t.text, fontSize: 11, whiteSpace: 'nowrap' }}>{(d.purchase_price != null && d.purchase_price !== '') ? Number(d.purchase_price).toLocaleString() + '원' : '-'}</td>
          <td style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 600, color: d.current_qty === 0 ? t.red : t.text }}>{d.current_qty?.toLocaleString()}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left' }}>{isNonIns(d) ? <Bd bg={t.blueL} color={t.blue}>비보험</Bd> : <span style={{ fontSize: 10, color: t.textL }}>보험</span>}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left', whiteSpace: 'nowrap' }}>{sm ? <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 600, background: smBg, color: smFg, border: '1px solid ' + smFg + '33' }}>{sm}</span> : <span style={{ color: t.textL, fontSize: 10 }}>-</span>}</td>
          <td style={{ padding: '9px 12px', textAlign: 'left', fontSize: 11, color: d.storage_location ? t.text : t.textL }} title={d.storage_location ? '' : '보관위치 미입력'}>{d.storage_location || '—'}</td>
...
function ET({ items, color, editRow, editField, draftRef, startEdit, saveRow, saveNote, onEdit, unusedDays, isUnused, alertSt, ip2 }) { const { t } = useTheme(); const{so,TS,sk,sd,setSort}=useSort('expiry_date');const[hfV,setHfV]=useState({})
    ...
    return<StandardTable t={t} TS={TS} sk={sk} sd={sd} setSort={setSort} hf={hf} hscroll={{noLabel:true,ends:true}} fontSize={11} layout="fixed" minWidth={1400} colWidths={[96,200,70,70,90,72,78,100,104,104,80,100,100,76,60]} cols={[...cols.map(([k,h])=>({k,h,th:{whiteSpace:'nowrap'}})),{k:'',h:'',plain:true,th:{padding:'5px 4px'}}]}>
    <tbody>{rows.map((d,i)=>{const days=exD(d.expiry_date);const a=alertSt(days);const uDays=unusedDays(d);const isEd=editRow===d.drug_code;const uu=isUnused(d)
      return<tr key={i} style={{borderBottom:`1px solid ${t.border}`,background:uu?t.redL+'60':''}} onMouseEnter={e=>{if(!uu)e.currentTarget.style.background=t.glass}} onMouseLeave={e=>{if(!uu)e.currentTarget.style.background=''}}>
        ...
        <td style={{padding:'5px 6px',fontSize:10}}>{isEd&&editField==='last_used_dept'?<select defaultValue={d.last_used_dept||''} onChange={e=>{draftRef.current.last_used_dept=e.target.value}} style={{...ip2,width:85}}><option value="">선택</option><option>가정의학과</option><option>재활의학과1</option><option>신경과</option><option>기타</option></select>:<span style={{color:t.textM,cursor:'pointer'}} onClick={()=>startEdit(d,'last_used_dept')}>{d.last_used_dept?<span style={{background:t.accentL,color:t.accent,padding:'1px 6px',borderRadius:4,fontSize:9,fontWeight:600}}>{d.last_used_dept}</span>:<span style={{color:t.textL,fontSize:9}}>클릭</span>}</span>}</td>
        <td style={{padding:'5px 6px',fontSize:10,textAlign:'center',width:100}}>{isEd&&editField==='last_used_date'?<DateCell key={d.drug_code} value={d.last_used_date||''} onChange={v=>{draftRef.current.last_used_date=v}}/>:<span style={{color:t.textM,cursor:'pointer',fontSize:10}} onClick={()=>startEdit(d,'last_used_date')}>{d.last_used_date||<span style={{color:t.textL,fontSize:9}}>클릭</span>}</span>}</td>
        ...
        <td style={{padding:'5px 6px',fontSize:10}}>{isEd&&editField==='recommended_action'?<select defaultValue={d.recommended_action||''} onChange={e=>{draftRef.current.recommended_action=e.target.value}} style={{...ip2,width:80}}>{REC_ACTIONS.map(a=><option key={a} value={a}>{a||'선택'}</option>)}</select>:<span style={{cursor:'pointer',fontSize:10}} onClick={()=>startEdit(d,'recommended_action')}>{d.recommended_action?<span style={{background:t.amberL,color:t.amber,padding:'1px 6px',borderRadius:4,fontSize:9,fontWeight:600}}>{d.recommended_action}</span>:<span style={{color:t.textL,fontSize:9}}>클릭</span>}</span>}</td>
        ...
        <td style={{padding:'5px 4px',whiteSpace:'nowrap'}}>{isEd&&<button onClick={()=>saveRow(d)} style={{padding:'2px 8px',borderRadius:4,border:`1px solid ${t.green}`,background:t.greenL,color:t.green,cursor:'pointer',fontSize:9,fontWeight:600,whiteSpace:'nowrap'}}>저장</button>}</td>
...
function ExpiryAlert({drugs,onEdit,focusLevel,onReload}){
  ...
  const[editRow,setEditRow]=useState(null);const[editField,setEditField]=useState(null);const draftRef=useRef({});const[resetKey,setResetKey]=useState(0)
  ...
  async function saveRow(d){
    const f=editField,v=draftRef.current[f],ud={}
    if(f==='last_used_dept')ud.last_used_dept=v||''
    else if(f==='last_used_date')ud.last_used_date=v||null
    else if(f==='recommended_action')ud.recommended_action=v||null
  ...
  function startEdit(d,field){setEditRow(d.drug_code);setEditField(field);draftRef.current={last_used_dept:d.last_used_dept||'',last_used_date:d.last_used_date||'',recommended_action:d.recommended_action||''}}
    ...
    <div style={{background:t.card,borderRadius:12,border:`1px solid ${t.border}`,overflow:'hidden',backdropFilter:'blur(12px)'}}>
      ...
      <StandardTable t={t} TS={TS} sk={sk} sd={sd} setSort={(kk,dd)=>{setSort(kk,dd);setPage(1)}} hf={hf} grid layout="fixed" minWidth={1860} colWidths={CW} hscroll={{noLabel:true,ends:true}} cols={COLS.map(([k,h])=>({k,h,plain:!k,...(k==='drug_code'?{sticky:{left:0,w:96}}:k==='drug_name'?{sticky:{left:96}}:{})}))}>
        <tbody>{!paged.length?<tr><td colSpan={COLS.length} style={{padding:40,textAlign:'center',color:t.textL}}>없음</td></tr>:paged.map((d,i)=>{const days=exD(d.expiry_date);const nt=getNT(d);return<tr key={i} style={{borderBottom:`1px solid ${t.border}`}} onMouseEnter={e=>e.currentTarget.style.background=t.glass} onMouseLeave={e=>e.currentTarget.style.background=''}>
          ...
          <td style={cellR}>{d.purchase_price!=null&&d.purchase_price!==''?Number(d.purchase_price).toLocaleString():'-'}</td>
