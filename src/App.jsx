...
function DrugList({ drugs, navFilter: nf, onEdit, onReload, nonins }) {
  ...
  /* ── 한글 조합 중 표 높이 고정 (첫 클릭 무반응) ──────────────────────────
     계측(2026-09-05): 조합 중 표시 행 수가 줄며 문서 높이가 2085px 진동하고,
     브라우저가 스크롤을 조정해 클릭 좌표가 표 위쪽 ATC 도넛으로 밀렸다
     (mousedown 은 td, click 은 div.no-print 도넛 — 좌표 이동 실증).
     조합 중에만 컨테이너 높이를 잠가 진동 자체를 없앤다.
     ★ 평상시·인쇄에는 이 값이 존재하지 않는다(0). vh·svh·dvh 는 쓰지 않는다 — 함정 #11.
     ★ 대조군: 재고현황(도넛 없음)은 997px 진동에도 정상이었다. 이 화면만 적용한다. */
  const tblWrapRef = useRef(null)
  const [imeLockH, setImeLockH] = useState(0)
  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ background: t.card, borderRadius: 14, border: `1px solid ${t.border}`, overflow: 'visible', boxShadow: t.shadow }}>
      ...
      <div ref={tblWrapRef} className="cnc-ime-lock" style={{ overflow: 'hidden', borderBottomLeftRadius: 14, borderBottomRightRadius: 14, ...(imeLockH ? { minHeight: imeLockH } : null) }}><StandardTable t={t} TS={TS} sk={sk} sd={sd} setSort={(k,sdv)=>{setSort(k,sdv);setPage(1)}} hf={hf} hscroll={{noLabel:true,ends:true}} cols={drugCols} colWidths={drugColWidths}>
