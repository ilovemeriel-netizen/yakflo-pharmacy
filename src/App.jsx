...
function DrugList({ drugs, navFilter: nf, onEdit, onReload, nonins }) {
  ...
  /* 검색 디바운스(대시보드 q/dq 250ms 패턴 재사용) + 한글 IME 조합 대기.
     search=입력 표시용(즉시), dsearch=필터·정렬·도넛 입력용(지연). 조합 중에는 타이머를 걸지 않아
     클릭의 mousedown이 input을 blur시킬 때 compositionend→필터 재계산이 끼어드는 것을 막는다(첫 클릭 유실 방지). */
  const [dsearch, setDsearch] = useState(''); const [composing, setComposing] = useState(false)
  useEffect(() => { if (composing) return; const h = setTimeout(() => setDsearch(search), 250); return () => clearTimeout(h) }, [search, composing])
  ...
  useEffect(() => { let on = true; supabase.from('location_vocab').select('label,sort_order,is_active').order('sort_order').then(({ data }) => { if (on) setLocOpts((data || []).filter(x => x.is_active !== false).map(x => x.label)) }); return () => { on = false } }, [])
  const filtered = so(drugs.filter(d => passesDrugFilters(d, { cats, stats, narcOnly, insF, atcF, search: dsearch }) && (!rxF || d.prescription_type === rxF) && (!hanoeOnly || d.narcotic_type === '한외마약') && (!donutF || (d[donutF.level] || '') === donutF.value) && (!cmpHF || d.compound_type === cmpHF) && (!stoHF || d.storage_method === stoHF) && (!locHF || (d.storage_location || '') === locHF) && (!hiAlertOnly || d.is_high_alert === true)))
  ...
  function _resetF(){applyCols(DRUG_DEFAULT_COLS);setSearch('');setDsearch('');setComposing(false);setCats(CATS);setStats(DEF_STATS);setNarcOnly(false);setHanoeOnly(false);setInsF(DEF_INS);setRxF(null);setDonutF(null);setCmpHF(null);setStoHF(null);setLocHF(null);setHiAlertOnly(false);setAtcF(null);setSort('drug_name','asc');setPage(1)}
