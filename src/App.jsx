...
function DrugList({ drugs, navFilter: nf, onEdit, onReload, nonins }) {
  ...
  /* 검색 디바운스 — 대시보드 q/dq 250ms 패턴과 완전히 동일.
     search=입력 표시용(즉시), dsearch=필터·정렬·도넛 입력용(지연).
     ★ 조합(composition) 게이트는 두지 않는다 — 한글은 마지막 음절이 계속 조합 중이라
       compositionend가 Enter·blur 전까지 오지 않아 실시간 검색이 죽는다(회귀 이력).
     첫 클릭 유실 방지는 디바운스만으로 성립: 클릭의 mousedown에서 compositionend→onChange가
     발생해도 재계산이 250ms 뒤로 밀려, 그 사이 mouseup이 끝나 click이 정상 발생한다. */
  const [dsearch, setDsearch] = useState('')
  useEffect(() => { const h = setTimeout(() => setDsearch(search), 250); return () => clearTimeout(h) }, [search])
  ...
  function _resetF(){applyCols(DRUG_DEFAULT_COLS);setSearch('');setDsearch('');setCats(CATS);setStats(DEF_STATS);setNarcOnly(false);setHanoeOnly(false);setInsF(DEF_INS);setRxF(null);setDonutF(null);setCmpHF(null);setStoHF(null);setLocHF(null);setHiAlertOnly(false);setAtcF(null);setSort('drug_name','asc');setPage(1)}
