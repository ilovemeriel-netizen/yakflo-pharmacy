  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ background: t.card, borderRadius: 14, border: `1px solid ${t.border}`, overflow: 'hidden', boxShadow: t.shadow }}>
      ...
      <div className="no-print" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', padding: '12px 18px', borderBottom: '1px solid ' + t.border }}>
        ...
        {luActive ? <button onClick={() => { setQ(''); setCatF(null); setCmpF(null); setStoF(null); setLocF(null); setInsF2(null); setAtcSel(null) }} style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid ' + t.border, background: 'transparent', color: t.textM, cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>초기화</button> : null}
      </div>
      <HScroll><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead><tr>{[['drug_code', '약품코드'], ['drug_name', '약품명'], ['category', '구분'], ['ingredient_en', '성분(EN)'], ['ingredient_kr', '성분(KR)'], ['atc_l1', 'ATC'], ['compound_type', '복합/단일'], ['manufacturer', '제조사'], ['current_qty', '현재고'], ['insurance_type', '급여'], ['storage_method', '보관'], ['storage_location', '위치'], ['expiry_date', '유효기한'], ['status', '상태']].map(([k, h]) => <th key={k} style={{ ...TS(k), background: t.bg, whiteSpace: 'nowrap', textAlign: k === 'current_qty' ? 'right' : 'left', minWidth: k === 'drug_name' ? 220 : undefined, ...(k === 'drug_code' ? { position: 'sticky', left: 0, zIndex: 6, minWidth: 128, maxWidth: 128, width: 128, borderRight: '1px solid ' + t.border } : k === 'drug_name' ? { position: 'sticky', left: 128, zIndex: 6, borderRight: '1px solid ' + t.border } : {}) }}><span onClick={() => hs(k)} style={{ cursor: 'pointer' }}>{h}<SI col={k} /></span>{hf[k] ? <HeaderFilter items={hf[k].items} value={hf[k].value} onChange={hf[k].on} color={hf[k].color} /> : null}</th>)}</tr></thead>
        <tbody>{!sorted.length ? <tr><td colSpan={14} style={{ padding: 32, textAlign: 'center', color: t.textL, fontSize: 12 }}>검색 결과 없음</td></tr> : sorted.map((d, i) => { const sm = d.storage_method || ''; const cold = sm.includes('냉장'); const shade = sm.includes('차광'); const smBg = cold ? t.blueL : shade ? t.amberL : t.bg; const smFg = cold ? t.blue : shade ? t.amber : t.textM; const cmp = d.compound_type || ''; const acc = atcColor(d.atc_l1); const atcChips = [d.atc_l1, d.atc_l2, d.atc_l3].filter(v => v && String(v).trim()); return <tr key={i} style={{ borderBottom: '1px solid ' + t.border, background: i % 2 ? t.bg : '' }} onMouseEnter={e => e.currentTarget.style.background = t.glass} onMouseLeave={e => e.currentTarget.style.background = i % 2 ? t.bg : ''}>
          <td style={{ padding: '9px 12px', fontSize: 10, color: t.textM, textAlign: 'left', whiteSpace: 'nowrap', position: 'sticky', left: 0, zIndex: 2, minWidth: 128, maxWidth: 128, width: 128, overflow: 'hidden', background: i % 2 ? t.bg : t.card, borderRight: '1px solid ' + t.border }}>{d.drug_code}<NT d={d} /></td>
...
function DrugList({ drugs, navFilter: nf, onEdit, nonins }) {
  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ background: t.card, borderRadius: 14, border: `1px solid ${t.border}`, overflow: 'hidden', boxShadow: t.shadow }}>
      ...
      <HScroll><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead><tr>{COLS.map(([k, h]) => <th key={k} style={{ ...TS(k), background: t.bg, whiteSpace: 'nowrap', textAlign: (k === 'current_qty' || k === 'purchase_price') ? 'right' : 'left', minWidth: k === 'drug_name' ? 220 : undefined, ...(k === 'drug_code' ? { position: 'sticky', left: 0, zIndex: 6, minWidth: 128, maxWidth: 128, width: 128, borderRight: '1px solid ' + t.border } : k === 'drug_name' ? { position: 'sticky', left: 128, zIndex: 6, borderRight: '1px solid ' + t.border } : {}) }}><span onClick={() => hs(k)} style={{ cursor: 'pointer' }}>{h}<SI col={k} /></span>{hf[k] ? <HeaderFilter items={hf[k].items} value={hf[k].value} onChange={hf[k].on} color={hf[k].color} /> : null}</th>)}</tr></thead>
...
function ColMenu({ colKey, sk, sd, setSort, filter }) {
  const { t } = useTheme();
  const [open, setOpen] = useState(false);
  const [pos, setPos] = useState({ top: 0, left: 0 });
  const ref = useRef(null), btnRef = useRef(null);
  function rect() { const el = btnRef.current; if (!el) return null; const r = el.getBoundingClientRect(); return { top: r.bottom + 4, left: Math.max(8, Math.min(r.left, window.innerWidth - 190)) } }
  function toggle(e) { e.stopPropagation(); if (!open) { const pp = rect(); if (pp) setPos(pp) } setOpen(o => !o) }
  useEffect(() => {
    if (!open) return;
    function place() { const el = btnRef.current; if (!el) return; const r = el.getBoundingClientRect(); if (r.bottom < 0 || r.top > window.innerHeight) { setOpen(false); return } setPos({ top: r.bottom + 4, left: Math.max(8, Math.min(r.left, window.innerWidth - 190)) }) }
    function onDoc(e) { if (ref.current && !ref.current.contains(e.target)) setOpen(false) }
    function onEsc(e) { if (e.key === 'Escape') setOpen(false) }
  ...
  const item = (onClick, label, on) => <button onClick={onClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', textAlign: 'left', padding: '6px 9px', border: 'none', background: on ? t.accent + '14' : 'transparent', color: on ? t.accent : t.text, cursor: 'pointer', fontSize: 11, fontWeight: on ? 700 : 500, borderRadius: 6, whiteSpace: 'nowrap' }} onMouseEnter={e => { if (!on) e.currentTarget.style.background = t.bg }} onMouseLeave={e => { if (!on) e.currentTarget.style.background = 'transparent' }}>{label}{on ? <span style={{ fontSize: 9 }}>✓</span> : null}</button>;
  return <span ref={ref} style={{ display: 'inline-block', marginLeft: 3, verticalAlign: 'middle', fontWeight: 400 }}>
    <span ref={btnRef} onClick={toggle} title="정렬·필터" style={{ cursor: 'pointer', fontSize: 10, fontWeight: 800, padding: '0 3px', borderRadius: 4, color: (active || fActive) ? t.accent : t.textL }}>▾</span>
    {open && <>
      <div style={{ position: 'fixed', inset: 0, zIndex: 9998 }} onClick={e => { e.stopPropagation(); setOpen(false) }} />
      <div onClick={e => e.stopPropagation()} style={{ position: 'fixed', top: pos.top, left: pos.left, zIndex: 9999, minWidth: 150, maxHeight: 300, overflowY: 'auto', background: t.cardSolid, border: '1px solid ' + t.borderH, borderRadius: 10, boxShadow: '0 12px 32px rgba(46,74,98,0.18)', padding: 6, textAlign: 'left' }}>
        ...
        {item(() => { setSort(colKey, 'asc'); setOpen(false) }, '오름차순 ▲', active && sd === 'asc')}
        {item(() => { setSort(colKey, 'desc'); setOpen(false) }, '내림차순 ▼', active && sd === 'desc')}
        {active ? item(() => { setSort('', 'asc'); setOpen(false) }, '정렬 해제', false) : null}
        {filter ? <div style={{ borderTop: '1px solid ' + t.border, margin: '5px 0' }} /> : null}
        {filter ? <div style={{ fontSize: 9, color: t.textL, fontWeight: 700, padding: '2px 9px 4px' }}>필터</div> : null}
