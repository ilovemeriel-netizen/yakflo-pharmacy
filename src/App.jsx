  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
      ...
      <div onClick={() => onNav({ menu: 'stock' })} style={{ background: t.card, borderRadius: 14, padding: '18px 22px', border: `1px solid ${t.border}`, boxShadow: t.shadow, cursor: 'pointer', transition: 'all .15s' }} onMouseEnter={hv} onMouseLeave={hx}>
        ...
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${t.border}` }}><div style={{ fontSize: 11, color: t.textM, marginBottom: 6 }}>📊 재고 구성</div>{(() => { const _sg = [{ l: '사용', v: s.active, c: t.green }, { l: '휴면', v: s.dormant, c: t.amber }, { l: '중지', v: s.stopped, c: t.textM }]; const _tt = _sg.reduce((a, x) => a + x.v, 0) || 1; return <><div style={{ display: 'flex', height: 8, borderRadius: 4, overflow: 'hidden', background: t.bg }}>{_sg.map((x, i) => x.v > 0 ? <div key={i} title={x.l + ' ' + x.v} style={{ width: (x.v / _tt * 100) + '%', background: x.c }} /> : null)}</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 6, fontSize: 10, color: t.textM }}>{_sg.map((x, i) => <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: x.c, display: 'inline-block' }} />{x.l} {x.v}</span>)}</div></>; })()}</div>
