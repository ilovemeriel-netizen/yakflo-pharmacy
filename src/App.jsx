  ...
  return <div>
    ...
    <div className="atc-print-only">
      {[0, 1].map(pg => { const rows = pg === 0 ? slot91Ga.slice(0, 46) : slot91Ga.slice(46); return <div key={pg} className="atc-page">
        ...
        <tbody>{rows.map(({ slot, d }, i) => <tr key={i}><td className="sl" style={{ fontWeight: 700, borderBottom: '0.3pt solid ' + t.border }}>{slot}</td><td className="nm" style={{ fontWeight: 600, borderBottom: '0.3pt solid ' + t.border }} title={d.drug_name}>{d.drug_name}</td><td className="ig" style={{ color: t.textM, borderBottom: '0.3pt solid ' + t.border }} title={d.ingredient_kr || ''}>{d.ingredient_kr || ''}</td><td className="bd" style={{ borderBottom: '0.3pt solid ' + t.border }}>{dBadges(d).filter(b => b.x !== '상비').map((b, bi) => <span key={bi} style={badgeStyle(b.bg, b.fg, b.bd)}>{b.x}</span>)}</td></tr>)}</tbody></table>
