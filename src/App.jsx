...
function Dashboard({ drugs, inv, txns, onNav, onEdit }) {
  ...
  return <div style={{ padding: '20px 24px' }}>
    ...
    <div style={{ background: t.card, borderRadius: 14, border: `1px solid ${t.border}`, overflow: 'hidden', boxShadow: t.shadow }}>
      <div onClick={() => onNav({ menu: 'druglist' })} title="약품목록으로 이동" style={{ padding: '14px 22px', borderBottom: `1px solid ${t.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: t.accentL, cursor: 'pointer', transition: 'background .15s' }} onMouseEnter={e => e.currentTarget.style.background = t.accent + '22'} onMouseLeave={e => e.currentTarget.style.background = t.accentL}><span style={{ fontWeight: 700, fontSize: 14, color: t.accent, display: 'flex', alignItems: 'center', gap: 6 }}>💊 사용 중인 약품 <span style={{ fontSize: 11, fontWeight: 500, color: t.textM }}>→ 약품목록</span></span><span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>{_showReset ? <button onClick={e => { e.stopPropagation(); _resetF() }} title='필터 초기화' style={{ padding: '4px 10px', borderRadius: 8, border: '1px solid ' + t.accent, background: t.accent + '12', color: t.accent, cursor: 'pointer', fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap' }}>필터 초기화{_af > 0 ? ' (' + _af + ')' : ''}</button> : null}<span style={{ fontSize: 13, fontWeight: 700, color: t.accent }}>{s.active}개</span></span></div>
