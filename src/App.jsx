...
function AtcView({ drugs, onReload }) {
  ...
  const fspOnly = propCand.filter(d => !isN(d) && !d.atc_excluded && d.fsp_slot && !d.atc_slot); /* FSP 전용 — FSP만 배정·카세트 미보유 → 카세트 배정 제외(순위는 유지) */
  const assignable = propCand.filter(d => !isN(d) && !d.atc_excluded && !(d.fsp_slot && !d.atc_slot)); /* 슬롯 배정 대상(향정·수기제외·FSP전용 건너뛰기) */
  ...
  return <div>
    ...
    {viewMode === 'proposal' && <div className="no-print" style={{ background: t.card, border: '1px solid ' + t.border, borderRadius: 12, padding: 14, marginBottom: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
        {[{ tag: 'A', tagBg: t.greenL, tagFg: t.green, title: '신규 진입', note: '카세트 확보 필요', rows: [...aNew.map(d => ({ _slot: atcSlotKey(propSlotOf[d.drug_code]), slotLabel: propSlotOf[d.drug_code], name: d.drug_name, r3: usageScore(d), rank: rankFull.get(d.drug_code) || null, excl: false, reason: null, d })), ...manualEx.filter(d => (rankFull.get(d.drug_code) || 999) <= 88 && !curCodes2.has(d.drug_code)).map(d => ({ _slot: 9999, slotLabel: null, name: d.drug_name, r3: usageScore(d), rank: rankFull.get(d.drug_code) || null, excl: true, reason: d.atc_exclude_reason, d })), ...fspOnly.filter(d => (rankFull.get(d.drug_code) || 999) <= 88 && !curCodes2.has(d.drug_code)).map(d => ({ _slot: 9998, slotLabel: null, name: d.drug_name, r3: usageScore(d), rank: rankFull.get(d.drug_code) || null, excl: false, fsp: true, reason: d.fsp_slot, d }))] }, { tag: 'B', tagBg: t.amberL, tagFg: t.amber, title: '제외', note: '수동 조제 전환', rows: bOut.map(cc => { const dd = (drugs || []).find(x => x.drug_code === cc); return { _slot: atcSlotKey(dd && dd.atc_slot), slotLabel: dd && dd.atc_slot, name: (dd && dd.drug_name) || cc, r3: dd ? usageScore(dd) : 0, rank: rankFull.get(cc) || null, excl: false, reason: null, d: dd }; }) }].map(sec => <div key={sec.tag} style={{ border: '1px solid ' + t.border, borderRadius: 10, overflow: 'hidden' }}>
          ...
          <div style={{ padding: 8 }}><AtcTable t={t} minWidth={340} colWidths={[68, 148, 60, 56]} resetable qfilter={r => !r.d || matches(r.d)} rows={sec.rows} cols={[{ k: '_slot', h: '슬롯', align: 'center', filter: true, render: r => r.fsp ? B('FSP 전용', t.greenL, t.green, t.green) : r.excl ? B('수기 조제', t.amberL, t.amber, t.amber) : (r.slotLabel ? B(r.slotLabel, t.accentL, t.accent, t.accent) : <span style={{ color: t.textL }}>-</span>) }, { k: 'name', h: '약품명', align: 'left', render: r => (r.excl || r.fsp) ? <span>{r.name} <span style={{ color: t.textL, fontSize: 10 }}>({r.reason || (r.fsp ? 'FSP' : '수기')})</span></span> : r.name }, { k: 'r3', h: '3개월', align: 'right' }, { k: 'rank', h: '순위', align: 'right', render: r => r.rank == null ? '-' : r.rank }]} /></div>
        </div>)}
      </div>
