...
function StockStatus({drugs,inv,navFilter:nf,onEdit,onAdjust,onReload,onDispose}){
    ...
    const reader=new FileReader();reader.onload=ev=>{
      try{
        ...
        rows.forEach((r,i)=>{
          ...
          updates.push({code,upd:{prev_year_usage:pv,recent_3m_usage:rv,monthly_avg:m,safety_stock:m!=null?Math.max(Math.ceil(m*SAFETY_FACTOR),1):null,max_stock:m!=null?Math.round(m*3):null,usage_source:'manual'}})
        })
  ...
  function remapUnmatched(i){setURep(p=>{const it=p.unmatched[i];const nc=String(it.newCode??it.code).trim();const cs=new Set(drugs.map(d=>d.drug_code));if(!cs.has(nc))return{...p,unmatched:p.unmatched.map((x,j)=>j===i?{...x,err:'여전히 미매칭'}:x)};const pv=it.py??null,rv=it.r3??null;let m=null;if(rv!=null)m=Math.round(rv/3);else if(pv!=null)m=Math.round(pv/12);const upd={prev_year_usage:pv,recent_3m_usage:rv,monthly_avg:m,safety_stock:m!=null?Math.max(Math.ceil(m*SAFETY_FACTOR),1):null,max_stock:m!=null?Math.round(m*3):null,usage_source:'manual'};return{...p,updates:[...p.updates,{code:nc,upd}],unmatched:p.unmatched.filter((_,j)=>j!==i)}})}
